...
describe("SSMProviderTest", () => {
    ...
    function stubSSMResult(key: string, result: SSMProvider.Value | undefined) {
        dataLoader.load.withArgs(key).resolves(result);
    }
    ...
    it("not available if ssm key does not exist", () => {
        ...
        return expect(provider.getValue()).resolves.toEqual(left(new ValueNotAvailable(`SSM: ${KEY}`)));
    });
    ...
    it("success", () => {
        ...
        return expect(provider.getValue()).resolves.toEqual(right(RESULT));
    });

    it("forwards error from dataloader", () => {
        const error = new Error("test");
        dataLoader.load.withArgs(KEY).rejects(error);
        const provider = new SSMProvider(KEY, dataLoader);

        return expect(provider.getValue()).resolves.toEqual(left(error));
    });
});
