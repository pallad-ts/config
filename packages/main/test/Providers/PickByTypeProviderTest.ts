...
describe('PickByTypeProvider', () => {
    ...
    describe.each([
        ...
        it('not available if type is not available', () => {
            const provider = PickByTypeProvider.create(
                new DummyProvider({isAsync: isTypeAsync, description: 'type'})
            )
        ...
        it('fails if resolved type is not available', () => {
            const provider = PickByTypeProvider.create(
                new DummyProvider({isAsync: isTypeAsync, value: 'baz'})
            )
                .registerOptions(TYPE_1, optionsProvider1)
                .registerOptions(TYPE_2, optionsProvider2);
        ...
        describe('forwards error', () => {
            ...
            it('from type', () => {
                const provider = PickByTypeProvider.create(
                    new DummyProvider({isAsync: isTypeAsync, error})
                )
                    .registerOptions(TYPE_1, optionsProvider1)
                    .registerOptions(TYPE_2, optionsProvider2);
    ...
    it('types', () => {
        ...
        type Providers = PickByTypeProvider.Value<'foo', typeof OPTIONS_1> |
