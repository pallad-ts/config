...
describe("loadConfig", () => {
    it("simple object without providers", () => {
        ...
        expect(result).toEqual(right(config));
    });
    ...
    it("forwards provider value with multiple errors", () => {
        ...
        const result = loadConfig(config);
        expect(result).toEqual(left([error1, error2]));
    });
    ...
    describe("with providers", () => {
        describe.each([
            ...
            it("all available", () => {
                ...
                const result = loadConfig(config);
                (isAsync ? expect(result).resolves : expect(result)).toStrictEqual(
                    right({
                        foo: VALUE_1,
                        tab: [VALUE_2, 10],
                        object: {
            ...
            describe("fails if any of providers fails", () => {
                ...
                it("first fail as error", () => {
                    ...
                    (isAsync ? expect(result).resolves : expect(result)).toStrictEqual(left([error]));
                });
                ...
                it("all fail as error", () => {
                    ...
                    (isAsync ? expect(result).resolves : expect(result)).toStrictEqual(left([error, error, error]));
                });
                ...
                it("first fail as not available", () => {
                    ...
                    (isAsync ? expect(result).resolves : expect(result)).toStrictEqual(
                        left([new ValueNotAvailable("prov 1")])
                    );
                });
                ...
                it("second fail as not available", () => {
                    ...
                    (isAsync ? expect(result).resolves : expect(result)).toStrictEqual(
                        left([new ValueNotAvailable("prov 2")])
                    );
                });
                ...
                it("third fail as not available", () => {
                    ...
                    (isAsync ? expect(result).resolves : expect(result)).toStrictEqual(
                        left([new ValueNotAvailable("prov 3")])
                    );
                });
